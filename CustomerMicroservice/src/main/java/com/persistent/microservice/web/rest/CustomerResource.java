package com.persistent.microservice.web.rest;

import com.codahale.metrics.annotation.Timed;
import com.persistent.microservice.domain.Customer;
import com.persistent.microservice.rabbitmq.RabbitMqConfiguration;
import com.persistent.microservice.service.CustomerService;
import com.persistent.microservice.utils.MicroserviceUtils;
import com.persistent.microservice.web.rest.util.HeaderUtil;
import com.persistent.microservice.web.rest.util.PaginationUtil;
...
public class CustomerResource {
    ...
    public CustomerResource(CustomerService customerService,RabbitTemplate rabbitTemplate) {
    ...
    public ResponseEntity<Customer> createCustomer(@Valid @RequestBody Customer customer) throws URISyntaxException {
        ...
        JSONObject jSendMessage = new JSONObject();
        jSendMessage = new JSONObject();
		try {
			jSendMessage.put("customerId", new Long(result.getId()));
			jSendMessage.put("orderId", new Long(1));
			jSendMessage.put("totalamount", new Double(100));
			jSendMessage.put("eventType", MicroserviceUtils.CREATE_ORDER);
			rabbitTemplate.convertAndSend(RabbitMqConfiguration.ORDER_TO_CUSTOMER_QUEUE, jSendMessage.toString());
